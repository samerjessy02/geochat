...
def ingest_file(filename: str, raw_bytes: bytes, display_name: str) -> dict:
    ...
    if ext in ("geojson", "json"):
        ...
        raise IngestError(f"Unsupported file type: .{ext}. Upload a .geojson or .csv file.")
    ...
    # normalize geometry column name PostGIS/geoalchemy convention used elsewhere in this project
    gdf = gdf.rename_geometry("wkb_geometry")

    # sanitize column names: lowercase, no spaces/special chars, avoid reserved words
    rename_map = {}
    for col in gdf.columns:
        if col == "wkb_geometry":
            continue
        clean = "".join(c.lower() if c.isalnum() else "_" for c in col).strip("_")
        clean = clean or "col"
    ...
    try:
        ...
        raise IngestError(f"Failed to write dataset to database: {e}")
    ...
    columns = [
        {"column_name": c, "data_type": str(gdf[c].dtype)}
        for c in gdf.columns if c != "wkb_geometry"
    ]
